...
class EmotionObject{
    ...
    getHtmlContainerString(){
        ...
        for(let i = 0; i < wordList.length; i++) {                           
            contents += `
                <div class="bg-white d-flex col-md-5 col-10 align-items-center m-2 p-1 col-5">
                    <div class="d-flex row col-9 p-2">
                        <p><strong>${wordList[i].word}</strong></p>
                        <p>${wordList[i].defintion}</p>
                    </div>
                        <img class="w-25 w-md-50" src="${wordList[i].pictureUrl}">
                </div>`;
        }
        return contents;
    }
...
for (let i = 0; i < emotions.length; i++) {
    topPage += `
    <div class="col-md-3 col-8 d-flex row m-1 py-4" style="background-color: ${emotions[i].color}">
        <div class="d-flex flex-wrap justify-content-center align-items-center　">
            <a href="#${emotions[i].emotion}" class="text-white pb-3"><strong>${emotions[i].emotion.toUpperCase()}</strong></a>
            <div class="d-flex justify-content-center">
                <img class="w-50" src="${pictureDictionary[emotions[i].onomatopoeia[0]]}">
            </div>
        </div>
    </div>`;       
}
...
for(let i = 0; i < emotions.length; i++) {
    topPage += `
    <div id="${emotions[i].emotion}" class="big-square p-4" style="background-color: ${emotions[i].color}">
